...
class Point(object):
    ...
    def move_to(self, x, y):
        self.distance_travelled = self.distance_travelled + math.sqrt((self.x - x)**2+(self.y - y)**2)
        self.x = x
        self.y = y
        self.total_moves_made = self.total_moves_made+1
    ...
    def move_by(self, dx, dy):
        ...
        self.y = self.y + dy
        self.total_moves_made = self.total_moves_made+1
        self.distance_travelled = self.distance_travelled + math.sqrt((self.x-(self.x-dx))**2+(self.y-(self.y-dy))**2)

    def get_number_of_moves_made(self):
        return self.total_moves_made
    ...
    def halfway_to(self, p):
        new_x = 0.5*(self.x + p.x)
        new_y = 0.5*(self.y + p.y)
